...
struct GenesisEmulatorView: View {
    let rom: ROMItem
    ...
    @StateObject private var core = PicoDriveCore()
    ...
    var body: some View {
        Group {
            ZStack {
                if !isAirPlayConnected {
                    PicoDriveScreenLayout(core: core)
                        .screenOverlay {
                            if isPlayingBIOS {
                                BIOSIntroView {
                                    // On Finish
                ...
                if !gameController.isControllerConnected {
                    PicoDriveVirtualController(isTransparent: true)
                        .zIndex(10)
                }
        ...
        .onAppear {
            ...
            Air.connection { connected in
                DispatchQueue.main.async {
                    if self.isAirPlayConnected != connected {
                        ...
                        if connected {
                            print(" AirPlay Connected: Starting Genesis Render")
                            ...
                            let tvView = PicoDriveRenderView(renderer: core.renderer)
            ...
            // Start Input Polling Timer
             self.inputTimer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { _ in
                 ...
                 if let gamepad = GameControllerManager.shared.currentController?.extendedGamepad {
                     controllerHeld = ControllerMappingManager.shared.isPressed(GenesisAction.fastForward, gamepad: gamepad, console: rom.console.systemName)
                 }
            ...
            print(" [GenesisEmulatorView] Loading Genesis: \(fileURL.path)")
            
            let wasPlaying = MusicPlayerManager.shared.isPlaying
            
            if core.loadGame(url: fileURL) {
                print(" Native PicoDrive Core Started")
                isLoading = false
                
                // Mute Core initially for BIOS
                core.pause()
            } else {
                print(" Native PicoDrive Failed")
            }
        ...
        .onDisappear {
            ...
            print("Exiting Genesis Emulator - Stopping AirPlay")
            Air.stop()
            Air.clearListeners()
    ...
    private var fastForwardButton: some View {
        Group {
            if MDSkinManager.shared.currentSkin != nil {
                EmptyView()
            } else {
